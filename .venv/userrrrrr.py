...
applications_manager = ApplicationsManager(bot)
...
def handle_confirm_application(call):
    ...
    if call.from_user.id in questionnaire_flow.user_states:
        ...
        bot.send_message(
            ...
            "Вы можете:\n"
            "• Просмотреть свои заявки в разделе 'Мои заявки'\n"
            "• Изучить другие студии в каталоге",
            parse_mode='HTML',
            reply_markup=markup
        )
...
def handle_select_answer_callback(call):
